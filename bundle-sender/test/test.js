...
describe("Wallet", function () {
  ...
  it("Send 0.01 ETH to each address.", async function () {
    ...
    const multisend_tx = await wallet
      .connect(payer)
      .batchETHTransfer(
    ...
    expect(final_balance[0]).to.equal(
      initial_balance[0].add(ethers.utils.parseEther("0.01"))
    );
    expect(final_balance[1]).to.equal(
      initial_balance[1].add(ethers.utils.parseEther("0.01"))
    );
  });
  ...
  it("Send 10 LINK to each address.", async function () {
    ...
    const approval_tx = await token
      .connect(payer)
      .approve(wallet.address, 20000);
    await approval_tx.wait();

    console.log("Payer approved the wallet for transfers");

    initial_balance = [
      await token.balanceOf(payee1.address),
  ...
  it("Insufficient total ETH provided", async function () {
    ...
    const batchTransfer_tx = await wallet
      .connect(payer)
      .batchETHTransfer(
        [payee1.address, payee2.address],
        [ethers.utils.parseEther("0.01"), ethers.utils.parseEther("0.01")],
  ...
  it("Angry token!", async function () {
    ...
    console.log(
      "Initial balance of the payer:",
      await dummy_token.balanceOf(payer.address)
    );

    // payer approves the contract
    const approval_tx = await dummy_token
      .connect(payer)
      .approve(wallet.address, 300);
    await approval_tx.wait();

    console.log("Payer approved the wallet for transfers");

    initial_balance = [
      await dummy_token.balanceOf(payee1.address),
